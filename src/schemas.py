...
VALID_HUMAN_ACCEPTANCE = {"接受", "部分接受", "不接受"}
VALID_RELIABILITY_LEVELS = {"高可信", "较可信", "存疑", "较不可信", "不可信", "证据不足"}
HUMAN_LEVEL_PLACEHOLDER = "--请选择--"
def validate_human_override_payload(override: HumanOverride) -> str | None:
    if not getattr(override, "acceptance", None) or override.acceptance not in VALID_HUMAN_ACCEPTANCE:
        return "请先选择是否接受 AI 结论，并选择人工最终可靠性等级。"
    if not getattr(override, "final_reliability_level", None) or override.final_reliability_level == HUMAN_LEVEL_PLACEHOLDER or override.final_reliability_level not in VALID_RELIABILITY_LEVELS:
        return "请先选择是否接受 AI 结论，并选择人工最终可靠性等级。"
    return None
